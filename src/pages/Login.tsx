...
export default function Login() {
  ...
  async function handlePostAuth(firebaseUser: FirebaseUser) {
    console.log('[Login] Starting handlePostAuth for:', firebaseUser.email);
    const token = await firebaseUser.getIdToken();
    setFirebaseUser(firebaseUser);
    setIdToken(token);

    try {
      console.log('[Login] Calling syncAuth (session)...');
      await syncAuth({
        id_token: token,
      });
      console.log('[Login] syncAuth completed successfully');
    } catch (err) {
      console.error('[Login] syncAuth failed:', err);
      // continue anyway to try and get profile
    }

    try {
      console.log('[Login] Calling getMe (profile)...');
      const profile = await getMe();
      console.log('[Login] getMe completed successfully');
      setUser(profile);
    } catch (err) {
      console.error('[Login] getMe failed:', err);
      // If getMe fails, user user might be stuck in loading state or partial state
    }
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          {/* Email/Password form — uses div role="form", NOT <form> */}
